...
unsigned long long absU (unsigned long long n)
{
	return n;
}
...
const std::vector<T> & get_primes (const T & b, T2 & pi)
{
	...
	else
	{
		...
		for (T cur=first; cur<=b; ++++cur)
		{
			...
			for (typename std::vector<T>::const_iterator iter = primes.begin(), end = primes.end();
				iter != end; ++iter)
			{
				const T & div = *iter;
				if (div * div > cur)
...
int main()
{
  ...
  //base maxPrime = 0;
	for (std::map <base, unsigned>::iterator i=m.begin(); i!=m.end(); ++i) {
    //if(maxPrime < i->first)
    //  maxPrime = i->first;
		std::cout << i->first;
    if(i->second > 1)
